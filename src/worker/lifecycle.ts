...
const COMMIT_MSG_HOOK_SH = `#!/bin/sh
# Strip agent CLI attribution from commit messages
FILE="$1"
TMP="$FILE.yk-tmp"
grep -v -E '^(Generated by \\[Cursor\\]|🤖 Generated with \\[Claude Code\\])' "$FILE" > "$TMP"
sed -e :a -e '/^[[:space:]]*$/{$d;N;ba' -e '}' "$TMP" > "$FILE"
rm -f "$TMP"
`;

/**
 * Installs a temporary git commit-msg hook that strips known agent CLI attribution lines.
 * Returns the hooks directory path for use with GIT_CONFIG core.hooksPath.
 */
export function installCommitMsgHook(workspaceId: string): string {
  const hooksDir = join("/tmp", `yes-kanban-hooks-${workspaceId}`);
  mkdirSync(hooksDir, { recursive: true });
  const hookPath = join(hooksDir, "commit-msg");
  writeFileSync(hookPath, COMMIT_MSG_HOOK_SH, { mode: 0o755 });
...
export async function runAgent(
  ...
  const hooksPath = installCommitMsgHook(String(workspaceId));
  const existingGitConfigCount = parseInt(cmd.env["GIT_CONFIG_COUNT"] ?? "0", 10);
  cmd.env["GIT_CONFIG_COUNT"] = String(existingGitConfigCount + 1);
  cmd.env[`GIT_CONFIG_KEY_${existingGitConfigCount}`] = "core.hooksPath";
  cmd.env[`GIT_CONFIG_VALUE_${existingGitConfigCount}`] = hooksPath;
